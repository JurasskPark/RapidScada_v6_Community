...
public class VTD
{
    ...
    public ulong GenerateRegisterAddress(ulong commandFunctionCode, ulong commandParametr, ulong commandRegisterAddress)
    {
        ulong result = 0;
        result = Convert.ToUInt64(commandFunctionCode.ToString() + Convert.ToString(commandParametr + commandRegisterAddress).PadLeft(5, '0'));
        return result;
    }

    public void GenerateTimeParametr(DateTime dateTimeNow, ushort FunctionCode, out ushort Parametr2, out ushort Parametr3)
    {
        Parametr2 = 0;
        Parametr3 = 0;
        ...
        int RegisterStartAddressHighByte = 0;
        int RegisterStartAddressLowByte = 0;

        int RegisterCountHighByte = 0;
        int RegisterCountLowByte = 0;

        if (FunctionCode == 84)
        {
            RegisterStartAddressHighByte = Day;
            RegisterStartAddressLowByte = Month;
            RegisterCountHighByte = 0;
            RegisterCountLowByte = 0;
        }
        else if (FunctionCode == 85)
        {
            RegisterStartAddressHighByte = Day-1;
            RegisterStartAddressLowByte = Month;
            RegisterCountHighByte = 1;  //count days
            RegisterCountLowByte = 0;
        }
        else if (FunctionCode == 86)
        {
            RegisterStartAddressHighByte = Month;
            RegisterStartAddressLowByte = Year - 2000;
            RegisterCountHighByte = 1;  //count mounth
            RegisterCountLowByte = 0;
        }

        Parametr2 = HEX_WORD.FromBytes(Convert.ToByte(RegisterStartAddressLowByte), Convert.ToByte(RegisterStartAddressHighByte));
        Parametr3 = HEX_WORD.FromBytes(Convert.ToByte(RegisterCountLowByte), Convert.ToByte(RegisterCountHighByte));
    }

    #region Формирование запроса по протоколу VTD
    public byte[] CalculateSendData(ushort Address, ushort FunctionCode, ushort Parametr1, ushort Parametr2, ushort Parametr3, ushort[] Value, bool CurrentValue = false)
    {
        byte[] byte_Frame = new byte[0];
        
        if(FunctionCode == 84 || FunctionCode == 85 || FunctionCode == 86)
        {
            if(CurrentValue == true)
            {
                DateTime dateTimeNow = DateTime.Now;
                GenerateTimeParametr(dateTimeNow, FunctionCode, out Parametr2, out Parametr3);
                byte_Frame = Read((ushort)Address, (ushort)FunctionCode, (ushort)Parametr1, (ushort)Parametr2, (ushort)Parametr3);
                return byte_Frame;
            }
            else
            {
    ...
    private byte[] Read(ushort Address, ushort FunctionCode, ushort Parametr1, ushort Parametr2, ushort Parametr3)
    {
        byte[] frame = new byte[10];
        frame[0] = (byte)(Address);
        frame[1] = (byte)(FunctionCode);
        frame[2] = (byte)(Parametr1 >> 8);
        frame[3] = (byte)(Parametr1);
        frame[4] = (byte)(Parametr2 >> 8);
        frame[5] = (byte)(Parametr2);
        frame[6] = (byte)(Parametr3 >> 8);
        frame[7] = (byte)(Parametr3);
    ...
    #endregion Формирование запроса по протоколу VTD

    #region Формирование ответа по запроса VTD
    ...
    //                {
    //                    byte[] bytes = BitConverter.GetBytes(Device.GetUshortHoldingRegister(Convert.ToUInt64((ulong)RegisterStartAddress + (ulong)index)));
    ...
    //                            byte[] bytes = BitConverter.GetBytes(Device.GetHoldingRegister(Convert.ToUInt16(tmpAddress)));
    ...
    //                    for (int index = 0; index < (int)RegisterCount; ++index)
    //                    {
    //                        byte[] bytes = BitConverter.GetBytes(Device.GetHoldingRegister(Convert.ToUInt16((int)RegisterStartAddress + index)));
    ...
    //                for (int index = 0; index < (int)RegisterCount; ++index)
    //                {
    //                    byte[] bytes = BitConverter.GetBytes(Device.GetInputRegister(Convert.ToUInt16((int)RegisterStartAddress + index)));
    //                    bufferSender[index * 2 + 3] = bytes[1];
    //                    bufferSender[index * 2 + 4] = bytes[0];
    //                }
    ...
    //                        {
    //                            int tmpAddress = ((int)RegisterStartAddress + index + (AddressCount % 2)) / 2;
    //                            byte[] bytes = BitConverter.GetBytes(Device.GetInputRegister(Convert.ToUInt16(tmpAddress)));
    //                            bufferSender[index * 2 + 3] = bytes[3];
    //                            bufferSender[index * 2 + 4] = bytes[2];
    //                        }
    //                        else
    ...
    #endregion Формирование ответа по запроса VTD
    ...
    public byte[] DecodeData(byte[] bufferReceiver)
    {
        ...
        Array.Copy(bufferReceiver, 3, NumArrayRegisters, 0, (int)bufferReceiver[2]);
