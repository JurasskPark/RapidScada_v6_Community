...
using System.Threading.Tasks;
    ...
    public class MasterDeviceReceiverSender
    {
        ...
        public static byte[] Sender(int typserver, Guid channelid, byte[] bufferReceiver, ref string Message)
        {
            ...
            {
                tmp_Address = (int)bufferReceiver[0];
            ...
            {
                tmp_Address = (int)bufferReceiver[6];
            ...
            else if (typserver == 3) //ModbusASCII
            {
                ...
                tmp_Address = (int)bufferReceiver[1];
            ...
            else if (typserver == 4)
            {
                try
                {
                    ...
                    //ProjectChannel channel = HARDWARE_GROUP_MODBUS.Channels.Where(r => r.ID == channelid).FirstOrDefault();
                    //ProjectChannel channel = HARDWARE_GROUP_MODBUS.Channels.Where(r => r.ID == channelid).FirstOrDefault();
            ...
            if (typserver >= 1 && typserver <= 3)
            {
                ...
                //Поиск
                Message += "Поиск устройства с адресом " + tmp_Address.ToString() + "." + Environment.NewLine;

                //ProjectDevice Device = ProjectChannel.Devices.Find((Predicate<ProjectDevice>)(r => r.Address == tmp_Address && r.ID == channelid));
                //if (Device == null)
                //{
                //    //Устройство не найдено
                //    Message += "Устройство с адресом " + tmp_Address.ToString() + " не найдено." + Environment.NewLine;
    ...
    internal class SerialPortOpros
    {
        ...
        public byte[] Opros(ProjectChannel channel)
        {
            ////////////////Переменная
            byte[] bufferSender = (byte[])null;
            string tmp_bufferSender = string.Empty;
