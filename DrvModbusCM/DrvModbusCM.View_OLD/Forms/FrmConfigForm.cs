    ...
    public partial class FrmConfigForm : Form
    {
        ...
        private void FormProperties(string configDir, int deviceNum)
        {
            ...
            if (deviceNum == 0)
            {
                ...
                {
                    ProjectSaveAs(driverConfigPath);
        ...
        private void ConfigToControls()
        {
            ...
            ProjectLoad(driverConfigPath);
            project.Load(driverConfigPath, out string errMsg);
        ...
        {
            if (String.IsNullOrEmpty(driverConfigPath))
            {
                if (appDirs != null)
                {
                    driverConfigPath = Path.Combine(appDirs.ConfigDir, DriverUtils.GetFileName(deviceNum));
                }
                else
                {
                    driverConfigPath = Path.Combine(AppDomain.CurrentDomain.BaseDirectory, DriverUtils.GetFileName(deviceNum));
                }
            }

            SerializeTreeView(trvProject, driverConfigPath);

            SaveDebug(driverConfigPath);
        }

        public void ProjectSaveAs(string driverConfigPath)
        {
            if (String.IsNullOrEmpty(driverConfigPath))
            {
                if (appDirs != null)
