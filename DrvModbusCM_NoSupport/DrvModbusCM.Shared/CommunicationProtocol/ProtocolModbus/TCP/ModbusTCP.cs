...
class ModbusTCP
{
    ...
    public byte[] CalculateSendData(ushort DeviceAddress, ushort FunctionCode, ushort RegisterStartAddress, ushort RegisterCount, ushort[] Value)
    {
        byte[] byte_Frame = new byte[0];
        if (FunctionCode == 1 || FunctionCode == 2 || FunctionCode == 3 || FunctionCode == 4)
        {
            byte_Frame = Read(FunctionCode, DeviceAddress, FunctionCode, RegisterStartAddress, RegisterCount);
            return byte_Frame;
        }
        else if (FunctionCode == 5 || FunctionCode == 6)
        {
            byte_Frame = Write(FunctionCode, DeviceAddress, FunctionCode, RegisterStartAddress, HEX_WORD.ToByteArray(Value));
            return byte_Frame;
        }
        else if (FunctionCode == 15)
        {
            ...
            byte_Frame = WriteMultipleCoils(FunctionCode, DeviceAddress, FunctionCode, RegisterStartAddress, RegisterCount, tmp_Values);
            return byte_Frame;
        }
        else if (FunctionCode == 16)
        {
            byte_Frame = WriteAll(FunctionCode, DeviceAddress, FunctionCode, RegisterStartAddress, HEX_WORD.ToByteArray(Value));
            return byte_Frame;
        }
    ...
    private byte[] Read(ushort ID, ushort DeviceAddress, ushort FunctionCode, ushort RegisterStartAddress, ushort RegisterCount)
    {
        ...
        frame[6] = (byte)(DeviceAddress);                   //Адрес устройства 
    ...
    private byte[] Write(ushort ID, ushort DeviceAddress, ushort FunctionCode, ushort RegisterStartAddress, byte[] Value)
    {
    ...
    private byte[] WriteAll(ushort ID, ushort DeviceAddress, ushort FunctionCode, ushort RegisterStartAddress, byte[] Values)
    {
    ...
    private byte[] WriteMultipleCoils(ushort ID, ushort DeviceAddress, ushort FunctionCode, ushort RegisterStartAddress, ushort RegisterCount, byte[] Values)
    {
        int size = Values.Length;
        byte[] frame = new byte[13 + size];
        frame[0] = (byte)(ID >> 8);
